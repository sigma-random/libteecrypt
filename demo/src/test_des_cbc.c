...
TEE_Result crypt_cipher_des_cbc(const void *message, size_t message_len,
...
int main(int argc, char **argv)
{
	...
	size_t key_len  = 8;				// 64bits = 8Bytes
	...
	res  = crypt_cipher_des_cbc((const void *)mess, mess_len, 
								(const void *)key, key_len, 
								cipher, &cipher_len,
								IV, IV_len,
								crypt_mode);					
	if(res != TEE_SUCCESS) {
		TEE_Printf("[err] crypt_cipher_des_cbc::TEE_MODE_ENCRYPT\n");
		return 0;
	}	
	if( cipher_len > 0) {
		TEE_Hexdump("crypt_cipher_des_cbc::plain", mess, mess_len, 16, true);
		TEE_Hexdump("crypt_cipher_des_cbc::key", key, key_len, 16, true);
		TEE_Hexdump("crypt_cipher_aes_cbc::IV", IV, IV_len, 16, true);
		TEE_Hexdump("crypt_cipher_des_cbc::cipher", cipher, cipher_len, 16, true);
	}



	TEE_Printf("\n============================TEE_MODE_DECRYPT============================\n");
	/* [0x03] Decrypt : des-cbc  */
	...
	res  = crypt_cipher_des_cbc((const void *)cipher, cipher_len, 
								(const void *)key, key_len, 
								plain, &plain_len,
								IV, IV_len,
								crypt_mode);					
	if(res != TEE_SUCCESS) {
		TEE_Printf("[err] crypt_cipher_des_cbc::TEE_MODE_DECRYPT\n");
		return 0;
	}	
	if( plain_len > 0) {
		TEE_Hexdump("crypt_cipher_des_cbc::cipher", cipher, cipher_len, 16, true);
		TEE_Hexdump("crypt_cipher_des_cbc::key", key, key_len, 16, true);
		TEE_Hexdump("crypt_cipher_des_cbc::IV", IV, IV_len, 16, true);
		TEE_Hexdump("crypt_cipher_des_cbc::plain", plain, plain_len, 16, true);
	}
								...
								uint32_t mode)
{
	...
	uint32_t algorithm    = TEE_ALG_DES_CBC_NOPAD;
	uint32_t maxKeySize   = 64;							// maxKeySize must be 64 for DES algorithm
