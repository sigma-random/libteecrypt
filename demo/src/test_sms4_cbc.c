...
TEE_Result crypt_cipher_sms4_cbc(const void *message, size_t message_len,
...
int main(int argc, char **argv)
{
	...
	size_t mess_len = 16;				// multiples of Aes algorithm key block_size (block_size of AES is 16 Bytes)

	unsigned char key[] = "\x01\x23\x45\x67\x89\xAB\xCD\xEF\xFE\xDC\xBA\x98\x76\x54\x32\x10";

	size_t key_len  = 16;				// 128bits = 16Bytes
	...
	res  = crypt_cipher_sms4_cbc((const void *)mess, mess_len, 
								(const void *)key, key_len, 
								cipher, &cipher_len,
								IV, IV_len,
								crypt_mode);					
	if(res != TEE_SUCCESS) {
		TEE_Printf("[err] crypt_cipher_sms4_cbc::TEE_MODE_ENCRYPT\n");
		return 0;
	}	
	if( cipher_len > 0) {
		TEE_Hexdump("crypt_cipher_sms4_cbc::plain", mess, mess_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_cbc::key", key, key_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_cbc::IV", IV, IV_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_cbc::cipher", cipher, cipher_len, 16, true);
	}



	TEE_Printf("\n============================TEE_MODE_DECRYPT============================\n");
	/* [0x03] Decrypt : sms4-cbc  */
	...
	res  = crypt_cipher_sms4_cbc((const void *)cipher, cipher_len, 
								(const void *)key, key_len, 
								plain, &plain_len,
								IV, IV_len,
								crypt_mode);					
	if(res != TEE_SUCCESS) {
		TEE_Printf("[err] crypt_cipher_sms4_cbc::TEE_MODE_DECRYPT\n");
		return 0;
	}	
	if( plain_len > 0) {
		TEE_Hexdump("crypt_cipher_sms4_cbc::cipher", cipher, cipher_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_cbc::key", key, key_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_cbc::IV", IV, IV_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_cbc::plain", plain, plain_len, 16, true);
	}
								...
								uint32_t mode)
{
	...
	uint32_t algorithm            =  TEE_ALG_SM_SMS4_CBC_NOPAD;
	uint32_t maxKeySize           =  16*8;						/* valid sizes only supports 128 bits*/
	...
	TEE_InitRefAttribute(&attr_list[0], attributeID , (void *)key, key_len );
