...
TEE_Result crypt_cipher_sms4_ebc(const void *message, size_t message_len,
...
int main(int argc, char **argv)
{
	...
	res  = crypt_cipher_sms4_ebc((const void *)mess, mess_len, 
								(const void *)key, key_len, 
								cipher, &cipher_len,
								crypt_mode);					
	if(res != TEE_SUCCESS) {
		TEE_Printf("[err] crypt_cipher_sms4_ebc::TEE_MODE_ENCRYPT\n");
		return 0;
	}	
	if( cipher_len > 0) {
		TEE_Hexdump("crypt_cipher_sms4_ebc::plain", mess, mess_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_ebc::key", key, key_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_ebc::cipher", cipher, cipher_len, 16, true);
	}



	TEE_Printf("\n============================TEE_MODE_DECRYPT============================\n");
	/* [0x03] Decrypt : aes-128-ecb  */
	...
	res  = crypt_cipher_sms4_ebc((const void *)cipher, cipher_len, 
								(const void *)key, key_len, 
								plain, &plain_len,
								crypt_mode);					
	if(res != TEE_SUCCESS) {
		TEE_Printf("[err] crypt_cipher_sms4_ebc::TEE_MODE_DECRYPT\n");
		return 0;
	}	
	if( plain_len > 0) {
		TEE_Hexdump("crypt_cipher_sms4_ebc::cipher", cipher, cipher_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_ebc::key", key, key_len, 16, true);
		TEE_Hexdump("crypt_cipher_sms4_ebc::plain", plain, plain_len, 16, true);
	}
								...
								uint32_t mode)
{
	...
	uint32_t algorithm            =  TEE_ALG_SM_SMS4_ECB_NOPAD;
